...
def run_ols_model(time,genotype,phenotype):
    ...
    stats = stat.pvalues.drop(['time','genotype[0]','genotype[1]'])
    pvals_time = stat.pvalues['time']
    pvals_genotype = min(stat.pvalues['genotype[0]'],stat.pvalues['genotype[1]'])
    pvals_timeMgenotype = min(stats)
    return stat.params, pvals_time,pvals_genotype,pvals_timeMgenotype
...
def run_time_dependence(output_file):
    ...
    for time,phenotypes in run_groups:
        for phenotype in phenotypes:
            ...
            coefs,pvals_time,pvals_genotype,pvals_timeMgenotype=run_ols_model(data[time].values,genotypes.values,data[phenotype].values)
