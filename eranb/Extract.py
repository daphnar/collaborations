...
allHumanpath='/Users/daphna/Downloads/reproduce/DFOut_revisions2'
humanByFigurePath='/Users/daphna/Downloads/reproduce/HumanExtract_revisions2.xlsx'
...
def extract(*args, **kwargs):
            ...
            if kwargs['diamond']:
                    ...
                    if kwargs['organism'] == 'mice':
                        ...
                        if kwargs['figure'] == 'WTvsSOD10':
                            allmicedf = pandas.read_pickle(
                                '/net/mraid08/export/jafar/Microbiome/Analyses/Daphna/ALSElinav/MiceRuns/newPipeline/KEGGGenesMappingFiles_HGF/KO_mat_by_KEGG_0.0001.df')
                            allmicedf.index = allmicedf.index.map(lambda x: x.split('~')[-1])
                            kwargs['thresholdKegg'] = 20 / ((5e6) * 0.8)
                            kwargs['KeepAll'] = True
                        else:
                            allmicedf = pandas.read_pickle(
                                '/net/mraid08/export/jafar/Microbiome/Analyses/Daphna/ALSElinav/MiceRuns/tmp/Raw/KEGGGenesMapFiles/KO_mat_by_KEGG_0.0001.df')
                        ...
                        if kwargs['cohort'] == 'PNP':
                            ...
                            # allmicedf=pandas.read_pickle('/net/mraid08/export/jafar/Microbiome/Analyses/Daphna/ALSElinav/HumanSamples/revisions/KEGGGenesMapFiles_reseq/KO_mat_by_KEGG_0.0001_reseq.df')
                            # allmicedf=pandas.read_pickle('/net/mraid08/export/jafar/Microbiome/Analyses/Daphna/ALSElinav/HumanSamples/revisions/tmp/tmp_Dafna_ALS/KEGGGenesMapFiles/KO_mat_by_KEGG_0.0001.df')
                            allmicedf = pandas.read_pickle(
                                '/net/mraid08/export/jafar/Microbiome/Analyses/Daphna/ALSElinav/HumanSamples/revisions2/tmp_Dafna_ALS/KO_mat_by_KEGG_0.0001.df')
                    ...
                    if kwargs['source'] == 'mpa':
                        if kwargs['figure'] == 'WTvsSOD10':
                            ...
                            allmicedf = pandas.read_pickle(os.path.join(organismPath, 'MPA.df'))
                    ...
                    if kwargs['source'] == 'count':
                        allmicedf = pandas.read_pickle(
                            os.path.join(organismPath, 'ReadCountDF.dat')).reset_index().set_index('uniting_key')
                    # if 'KeepAll' not in kwargs:
                    #     ########################################################MY ADDITIONS####################
                    #     countdf = pandas.read_pickle(
                    #         os.path.join('/Users/daphna/Downloads/reproduce/DFOut_HumanSamples/ReadCountDF.dat')).reset_index().set_index(
                    #         'uniting_key')
                    #     aboveThresholdSamples = countdf[countdf['PostSubSamp'] >= kwargs['threshold']].index
                    #     aboveThresholdSamples = aboveThresholdSamples.map(lambda x: x.replace('Sample_', ''))
                    ...
                    if kwargs['organism'] == 'human':
                        micedf = allmicedf.loc[list(set(miceByFigure.UserID.values))]  # miceByFigure.index]
                    else:
    ...
    if 'k0s' in kwargs:
        ...
        print("looking for: ", len(set(kwargs['k0s'])), "K0s, found ",len(k0s)," K0s")
        micedf=micedf[k0s]
...
if __name__=="__main__":
    ...
    k0 = pandas.read_csv(\
        '/net/mraid08/export/jafar/Microbiome/Analyses/Daphna/ALSElinav/MiceRuns/Analyses/KEGG/NAM_Kegg.csv'\
)['Kegg entry'].values
#     human= extract(figure='Human',source='kegg',genotype='Control',
#                   organism='human',k0s=k0,apply=np.mean,timepoint=None,diamond=True)#figure='AkkKEGG',timepoint='sort',
#     human = extract(figure='Human', source='mpa', genotype='ALS',
#                     organism='human', apply=np.mean, timepoint=None)
#     print human.head()
#     print human.shape

    # human = extract(figure='Human', source='kegg',
    #                 organism='human', k0s=k0,
    #                 diamond=True, cohort='PNP',apply=np.mean)  # figure='AkkKEGG',timepoint='sort',
    # print human.head()
    mice = extract(figure='Human', source='count', genotype='all',
                   organism='human')
